document.addEventListener('alpine:init', () => {
  Alpine.data('timetableApp', () => ({
    loaded: false,
    currentDay: '',
    theme: localStorage.getItem('timetableTheme') ||
    ...
    goToDay(offset) {
      const idx = this.days.indexOf(this.currentDay);
      const next = (idx + offset + this.days.length) % this.days.length;
      this.selectDay(this.days[next]);
    },
    ...
    applyTheme(t) {
      ...
      localStorage.setItem('timetableTheme', t);
    },
    ...
    withBreaks(periods) {
      ...
      for (let i = 0; i < periods.length; i++) {
        ...
        if (i < periods.length - 1) {
          ...
          if (gap > 5) {
            out.push([periods[i][1], periods[i + 1][0], 'BREAK', this.minsToLabel(gap)]);
          }
        }
      }
      return out;
    ...
    isOngoing(start, end) {
      const cur = this.currentMinutes;
      return cur >= this.minutesOf(start) && cur < this.minutesOf(end);
    },
    ...
    cardAccentStyle(key) {
      const accent = this.ACCENT && this.ACCENT[key];
      if (!accent) return '';
      return `background: ${accent[0]}`;
    },

    subjectName(key) {
      if (!this.SUBJECTS || !this.SUBJECTS[key]) return key;
      return this.SUBJECTS[key].name;
    },

    subjectLine(key) {
      if (!this.SUBJECTS || !this.SUBJECTS[key]) return '';
      return this.SUBJECTS[key].subLine;
    },

    subjectChip(key) {
      if (!this.SUBJECTS || !this.SUBJECTS[key]) return '';
      return this.SUBJECTS[key].chipLabel;
    },

    accentColors(key) {
      const d = this.ACCENT && this.ACCENT[key];
      return d ? d : ['#6E56CF', '#F0ECFA'];
    },
    ...
    showSaturdayCaution(day) {
      return day === 'Saturday' && this.currentDayName === 'Saturday' && !this.isSaturdayClassWeek;
    },
    ...
    isEventCompleted(ev) {
      try {
        ...
        return arr.indexOf(this.eventKey(ev)) >= 0;
    ...
    injectCalendarBadges() {
      ...
      this.events.forEach(ev => {
        ...
        let subjKey = null;
        for (const [k, v] of Object.entries(this.SUBJECTS || {})) {
          if (v.name === ev.subject) { subjKey = k; break; }
        }
        if (!subjKey) return;
    ...
    get dividerCount() {
      return (key, subCount) => {
        return key === 'LABCOMBO' ? (subCount || 1) : 1;
      };
    },
